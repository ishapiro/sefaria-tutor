            ...
            <table class="w-full text-base border-collapse">
              ...
              <tbody>
                <tr
                  ...
                  :key="i"
                  class="border-t-2 border-slate-300 hover:bg-slate-50 first:border-t-0"
                >
                  <td
                    class="px-3 py-3 text-gray-600 align-top text-center tabular-nums"
                    :title="`Occurs ${countWordInPhrase(translationData?.originalPhrase ?? '', row.word ?? '')} time(s) in original text`"
                  >
                    {{ countWordInPhrase(translationData?.originalPhrase ?? '', row.word ?? '') || '—' }}
                  </td>
                  <td class="px-3 py-3 text-gray-700 align-top whitespace-normal break-words max-w-[12rem]">
                    {{ row.wordTranslation ?? '—' }}
                  </td>
                  <td
                    class="px-3 py-3 text-right font-medium align-top whitespace-normal break-words"
                    :class="{
                      'cursor-pointer hover:bg-blue-100 hover:text-blue-700 rounded': row.word && row.word !== '—'
                    }"
                    style="direction: rtl"
                  ...
                  <td class="px-3 py-3 text-gray-700 align-top whitespace-normal break-words">
                    {{ row.hebrewAramaic ?? '—' }}
                  </td>
                  <td class="px-3 py-3 text-gray-700 align-top whitespace-normal break-words min-w-[140px] pl-4">
                    {{ row.wordRoot ?? '—' }}
                  </td>
                  <td class="px-3 py-3 text-gray-700 align-top whitespace-normal break-words pl-4">
                    {{ row.wordPartOfSpeech ?? '—' }}
                  </td>
                  <td class="px-3 py-3 text-gray-700 align-top whitespace-normal break-words pl-4">
                    {{ row.wordGender ?? '—' }}
                  </td>
                  ...
                  <td
                    class="px-3 py-3 text-right font-medium align-top whitespace-normal break-words pl-4"
                    style="direction: rtl"
                  >
                    {{ row.presentTenseHebrew ?? '—' }}
                  </td>
                  <td class="px-3 py-3 text-gray-700 align-top whitespace-normal break-words min-w-[200px] pl-4">
                    {{ row.grammarNotes ?? '—' }}
                  </td>
                </tr>
              </tbody>
