<template>
  <div class="container mx-auto p-4">
        ...
        <Card class="mb-4">
          <template #title>
            <div class="flex justify-between items-center">
              ...
              <Button icon="pi pi-arrow-left" @click="handleCloseBook" class="p-button-text" />
            </div>
          </template>
          <template #content>
            ...
            <div v-else class="book-content">
              ...
              <div v-else class="verses-flex">
                <div class="verses-header">
                  ...
                  <div class="hebrew-header">Select text for word-by-word translation with grammar notes</div>
                </div>
        ...
        <div v-else-if="translationData" class="space-y-6">
          ...
          <div>
            ...
            >
              ...
              >
                <template #body="{ data }">
                  <span class="font-['SBL_Hebrew'] text-gray-900 text-lg">{{ data.word }}</span>
                </template>
              </Column>
              <Column 
                field="hebrewAramaic" 
              ...
              >
                <template #body="{ data }">
                  <span class="text-gray-600">{{ data.grammarNotes || '-' }}</span>
                </template>
              </Column>
            </DataTable>
          </div>
    ...
    <!-- Raw Data Dialog -->
    <Dialog v-model:visible="showRawData" 
            :modal="true" 
            :closable="true" 
            :dismissableMask="true"
            :style="{ width: '80vw' }"
            header="Raw Translation Data">
      <div class="p-4">
        <pre class="bg-gray-50 p-4 rounded-lg overflow-auto max-h-[70vh]">{{ JSON.stringify(rawTranslationData, null, 2) }}</pre>
      </div>
    </Dialog>
  </div>
</template>
  ...
  methods: {
    ...
    async fetchBookContent(refOverride = null, stackOverride = null) {
      ...
      try {
        ...
        if (isTalmud && response.data) {
          ...
          log(this.debug, 'Complex book texts:', {
    ...
    async translate_with_openai(text) {
      try {
        ...
        const requestBody = {
          prompt: isTalmud ? `Translate this phrase from the Talmud to english:${text}` : `Translate this phrase to english:${text}`,
          model: 'gpt-3.5-turbo'
        };
